...
from ansible_collections.pxc.cloud.plugins.module_utils.identity import (sort_and_hash, stack_vm_get_blake)
...
def run_module():
    ...
    existing_blake_ids = [
        stack_vm_get_blake(vm) for vm in stack_vms if stack_vm_get_blake(vm) is not None
    ]
    ...
    result = dict(
        changed=True, vms_to_create=vms_to_create, vms_to_delete=vms_to_delete
    )
