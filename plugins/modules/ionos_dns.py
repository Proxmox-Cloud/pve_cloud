...
def run_module():
    ...
    # get zones
    response = requests.get("https://api.hosting.ionos.com/dns/v1/zones", headers=headers)
    ...
    for challenge in module.params['challenge_data_dns']:
        ...
        if module.params['cleanup']:
            # find acme records
            response = requests.get(f"https://api.hosting.ionos.com/dns/v1/zones/{zone_id}", headers=headers)
            ...
            for record_id in acme_record_ids:
                response = requests.delete(f"https://api.hosting.ionos.com/dns/v1/zones/{zone_id}/records/{record_id}", headers=headers)
            ...
            for chal in challenge['value']:
                ...
                # post txt record
                response = requests.post(f"https://api.hosting.ionos.com/dns/v1/zones/{zone_id}/records", json=[records_body], headers=headers)
