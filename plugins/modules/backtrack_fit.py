...
def calculate_remaining_memory_dynamic(vms_distribution, hosts):
    remaining_memory = [host["mem_total"] - host["reserved"] for host in hosts]  # Extract the memory values
    fitted_hosts = []
    ...
    for vm in vms_distribution:
        ...
        remaining_memory[max_index] -= vm_memory  # Subtract the VM memory from the selected host
        fitted_hosts.append(hosts[max_index]["hostname"]) # Map the VM index to the host's hostname
...
def run_module():
    ...
    nodes = [node for node in module.params['nodes_to_create'] if 'target_host' not in node[1]]
    ...
    for vm_distribution in permutations(nodes):
        remaining_memory, fitted_hosts = calculate_remaining_memory_dynamic(vm_distribution, hosts)
    ...
    best_distribution.extend([node for node in module.params['nodes_to_create'] if 'target_host' in node[1]])
    best_fitted_hosts.extend([node[1]["target_host"] for node in module.params['nodes_to_create'] if 'target_host' in node[1]])
