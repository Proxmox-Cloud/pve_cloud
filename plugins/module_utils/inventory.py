...
async def get_online_pve_hosts(loader, yaml_data):
    ...
    for pve in pve_inventory:
        try:
            online_jump_host = get_online_jump_host(pve_inventory, pve)
...
async def fetch_pve_cluster_info(pve_host_of_cluster):
    ...
    async with connect_host_async(
        ...
        fetch_tasks.append(
            fetch_task(conn, "pvesh get /cluster/ha/groups --output yaml", "ha_groups")
        )
...
async def get_cluster_map(inventory, online_pve_hosts):
    ...
    cluster_map = {
        ...
        for cluster_name, information in (await asyncio.gather(*cluster_fetch_tasks))
    }
    return cluster_map
...
def build_pve_inventory(inventory, yaml_data, online_pve_hosts, cluster_map):
    ...
    # host we use in the collection to run alembic migrations
    # dnsupdates, generally requires a loaded python env
    # this is needed for clusters accessible only via jump host
    # in that case we select the first target pve host as the executor
    pxc_executor_set = False

    for pve_host in online_pve_hosts:
        if (
            "target_pve_hosts" in yaml_data
            and pve_host.hostname not in yaml_data["target_pve_hosts"]
        ):
            display.display(
                f"Skipping {pve_host.hostname} as not specified in target_pve_hosts!"
            )
            continue
        ...
        if (
            pve_host.cluster_name + "." + pve_host.cloud_domain
            == yaml_data["target_pve"]
        ):
            inventory.add_host(host_fqdn, group="target_pve")

            if pve_host.jump_host and not pxc_executor_set:
                # add the same host as the central pxc-executor-host
        ...
        if pve_host.hostname in cluster.host_ha_groups:
            inventory.set_variable(
                host_fqdn, "pve_ha_groups", cluster.host_ha_groups[pve_host.hostname]
            )
    ...
    # fallback / default set the host to localhost
    if not pxc_executor_set:
        inventory.add_host("pxc-executor-host")
...
async def add_lxc_to_inv(inventory, online_pve_hosts, target_pve, vm):
    ...
    for pve_host in online_pve_hosts:
        if (
            pve_host.hostname == vm["node"]
            and pve_host.cluster_name + "." + pve_host.cloud_domain == target_pve
        ):
    ...
    async with connect_host_async(
        ...
        for attempt in range(max_retries):
            try:
                pct_ip_show_cmd = await pve_conn.run(
                    f"pct exec {vm['vmid']} -- ip --json a show", check=True
                )
                ...
                if ip is not None:
                    display.display(
                        f"Got ip {ip} for {vm['vmid']} on attempt {str(attempt+1)}"
                    )
                    break
...
async def add_qemu_to_inv(inventory, cluster, vm):
    ...
    async with connect_host_async(
        ...
        for attempt in range(max_retries):
            try:
                pvesh_qemu_ifaces = await pve_conn.run(
                    f"pvesh get /nodes/{vm['node']}/qemu/{vm['vmid']}/agent/network-get-interfaces --output-format json",
                    check=True,
                )
                ...
                for interface in interfaces["result"]:
                    for address in interface["ip-addresses"]:
                        try:
                            ...
                            if parsed in ipaddress.IPv4Network(
                                cluster.cluster_vars["pve_vm_subnet"], strict=False
                            ):
        ...
        if cluster.first_online_host.jump_host:
            inventory.set_variable(
                vm["name"],
                "ansible_ssh_common_args",
                f"-o ProxyJump=root@{cluster.first_online_host.jump_host}",
            )
...
async def include_stack(
        ...
        for vm in cluster.pvesh_vms:
            if "tags" in vm:
                ...
                if include_fqdn in tags:
                    if (
                        matched_cluster_fqdn is not None
                        and matched_cluster_fqdn != cluster_fqdn
                    ):
                        raise AnsibleError(
                            f"Stack to include is in multiple pve clusters {include_fqdn} - unexpected behaviour {cluster_fqdn}/{matched_cluster_fqdn}"
                        )
    ...
    for vm in include_vms:
        ...
        if vm["type"] == "lxc":
            ...
            add_to_inv_tasks.append(
                add_lxc_to_inv(inventory, online_pve_hosts, matched_cluster_fqdn, vm)
            )
        elif vm["type"] == "qemu":
            inventory.set_variable(vm["name"], "ansible_user", qemu_ansible_user)
            add_to_inv_tasks.append(
                add_qemu_to_inv(inventory, cluster_map[matched_cluster_fqdn], vm)
            )
        else:
...
async def init_plugin(loader, inventory, yaml_data):
        ...
        for stack in yaml_data["include_stacks"]:
            include_tasks.append(
                include_stack(
                    ...
                    stack["host_group"],
                    (
                        stack["qemu_ansible_user"]
                        if "qemu_ansible_user" in stack
                        else "admin"
                    ),
                )
            )
    ...
    stack_fqdn = (
        f"{yaml_data['stack_name']}.{target_cluster.cluster_vars['pve_cloud_domain']}"
    )
