...
7. goto your https://PUBLIC-IP:8006 and access the proxmox ui and create a new Linux VLAN in the System/Network tab. Name it vmbr0.2 and assign the static ip 10.0.0.1/24. This will be used by the proxmox cluster and the control node later
8. also create a network for vm communication: vmbr0.10 10.0.1.1/24
9. create proxmox cluster under Datacenter/Cluster on the management interface vmbr0.2, also under Datacenter/Storage enable "Snippets" for local storage
10. add your apt repos refresh and upgrade
11. now to make virtual machines have access to the internet we need to configure the proxmox host to act as a router (this is a hack for the demo system only)
```bash
...
iptables -t nat -A POSTROUTING -s 10.0.1.0/24 -o vmbr0 -j MASQUERADE # allows it for the vm net defined in step 7.
...
Since this is a dedicated system you need a machine / container with direct access to all vms and proxmox hosts. for that we manually create an lxc, as a control node for running ansible and terraform configuration (proxmox cloud does not yet support remote access). 

This lxc needs a nic on the management switch / vlan, aswell as the vmdata vlan.

```bash
# On your freshly created proxmox cluster in the gui goto storage and download a template of your choice (ubuntu recommended)

# create your control node container with a network interface on the vm data bridge vlan and either the hosts corresponding id / the opnsense as gateway
# (you also should give the lxc your ssh key), the opnsense is the 254, so give the control node 253 in this case
# you need to add an additional nic with as static ip on the management vlan after creating the lxc, give it the .254 address in the management network

# then connect to your lxc connect to it using this command:
ssh -L 8081:localhost:8080 -o ProxyJump=root@PROXMOX_HOST_IP root@STATIC_VM_DATA_IP.253
...
# generate ssh key for your private git repos and proxmox host access (add it to one host to /root/.ssh/authorized_keys)
